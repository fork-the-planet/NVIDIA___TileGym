...
def _llama4_rope_forward_ct(q, k, freqs_cis, BLOCK_SIZE=None, imag_sign=1.0):
    ...
    if BLOCK_SIZE is None:
        BLOCK_SIZE, _ = _select_kernel_meta(head_dim_half)
...
class Llama4RopeCuTileFunction(torch.autograd.Function):
...
@register_impl("liger.llama4_rope", backend="cutile")
def llama4_rope(
    q: torch.Tensor,
    k: torch.Tensor,
    ...
    **kwargs,
    ...
    return Llama4RopeCuTileFunction.apply(q, k, freqs_cis, BLOCK_SIZE)
